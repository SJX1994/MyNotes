...
int main() { // 通过两个类 阵营类 和 士兵类 考察 构造 解析 封闭 友元 this指针 常量 动态内存分配 面向对象
	int t;
	int m;
	...
	while (t--) { // 依次执行每轮测试
		printf("Case:%d\n", nCaseNo++); // 输出当前测试编号
		cin >> m; // 输入初始生命元数。（制造武士需要生命元。）
		for (int i = 0; i < WARRIOR_NUM; i++) // 生成5种战士
			cin>>Warrior::initialLifeValue[i]; // 5种战士的生命值
		RedHead.Init(0, m); // 传入 生命元数 红方阵营初始化
		BlueHead.Init(1, m); // 传入 生命元数 蓝方阵营初始化
		int nTime = 0;
		while (true) {
			int tmp1 = RedHead.Produce(nTime); // 红方生产
			int tmp2 = BlueHead.Produce(nTime); // 蓝方生产
			if (tmp1 == 0 && tmp2 == 0) // 直到返回的生产需求都是0
				break;
			nTime++;
		}
	}
