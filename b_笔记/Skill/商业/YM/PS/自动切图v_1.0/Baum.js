...
(function () {
  ...
  PsdToImage = (function () {
    ...
    PsdToImage.prototype.outputLayer = function (doc, layer) {
      ...
      // 双数像素导出
        ...
        // alert(fileName + " 宽 " + Number(Width) + " 高 " + Number(Height));
        var newWidth, newHeight;
        if(Width%2==1 || Height%2==1)
        {
          newWidth = Width%2==1 ? Width+1 : Width;
          newHeight = Height%2==1 ? Height+1 : Height;
          // alert(fileName + " 新宽 " + Number(newWidth) + " 新高 " + Number(newHeight));
          doc.resizeCanvas( Number(newWidth) , Number(newHeight), AnchorPosition.MIDDLECENTER);
        }
      // 双数像素导出
      fileName = Util.layerToImageName(layer);
      opt = Util.parseOption(layer.name.split("@")[1]);
      if (indexOf.call(fileNames, fileName) >= 0) {
        alert(fileName + "存在同名图层。 请更改图层名称。");
      }
      if (opt['slice'] === 'false') {
        fileName += "-noslice";
      }
  ...
  Util = (function () {
    ...
    Util.layerToImageNameLoop = function (layer) {
      ...
      {
        //alert(layer.name +" -- "+ theParent.name);
        if( layer.parent.parent != myDocument)
        {
          return layer.parent.parent.name + layer.parent.name + layer.name;
        }
        return theParent.name + layer.name;
      }
      else
      {
        return layer.name;
