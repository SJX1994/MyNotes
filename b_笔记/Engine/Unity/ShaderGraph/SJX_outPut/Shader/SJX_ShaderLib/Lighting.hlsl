...
half3 LightingPhysicallyBased(BRDFData brdfData, half3 lightColor, half3 lightDirectionWS, half lightAttenuation, half3 normalWS, half3 viewDirectionWS)
{
    ...
    // half3 SJX_addColor = lerp(visibility,half3(0.0,1.0,0.0),1.0-visibility);
    return DirectBDRF(brdfData, normalWS, lightDirectionWS, viewDirectionWS) * radiance ;//+ SJX_addColor;
}

half3 SJX_getShadowMask(float4 shadowCoord ,half3 normalWS)
{
    Light mainLight = GetMainLight(shadowCoord);
   float lightAttenuation = MainLightRealtimeShadow(shadowCoord);
