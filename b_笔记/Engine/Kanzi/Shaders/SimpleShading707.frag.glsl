...
uniform highp mat4 CameraNormalMatrix;
...
//pack
    ...
    // lightcolor *= fCol;
    fCol = mix(vec3(0.0),m_texColor(Texture).rgb*(color*2.5),maskX)*max(scanMask,0.72);
        //fCol = vec3(scanMask);
        return fCol;
...
void main()
{
    ...
    //基础变量
        ...
        //基础颜色
        vec4 matCol = m_texColor(Texture)*BaseColor;
    ...
    //遮罩们
        ...
        float objmas = smoothstep(-28.1,-18.1,1.0-m_objectPos.z) -  smoothstep(19.0,26.0,1.0-m_objectPos.z);
        float objmas2 = smoothstep(-25.0,-23.0,1.0-m_objectPos.z) -  smoothstep(23.5,24.9,1.0-m_objectPos.z);
        float objmas3 = smoothstep(-25.0,-20.0,1.0-m_objectPos.z) -  smoothstep(15.5,22.9,1.0-m_objectPos.z);
        float objmas3_1 = smoothstep(-25.0,-20.0,1.0-m_objectPos.z) -  smoothstep(25.5,30.9,1.0-m_objectPos.z);
      ...
      //  float SlashL =Saturate( Saturate( step(0.0,-objectPos.y+3.0 + (objectPos.z/2.3+9.6)  )) - Saturate(step(0.0,objectPos.y-1.5+ (objectPos.z/29.3+0.6) )) )*2.0* step(0.0,-objectPos.y+2.2 - (objectPos.z/3.0+3.6)   ) ;
         float SlashL =Saturate(1.0- Saturate(smoothstep(0.7,0.9,objectPos.y + (objectPos.z/20.3)) ) ) ;
    ...
    //空间反射3 (白色)    
        ...
        vec4 addbright = vec4(1.0);
        addbright.rgb = Saturate(objectPos.y-11.5) *step(0.8,N.y) * cubox4.rgb; 
        cubox4
        = (1.0-frontMask)
        * NzMask
        * cubox4;
      
        lighting = m_Add2Light (lighting,Spec,cubox4+addbright*1.5,matCol,0.7);
       // lighting.rgb = vec3(objmas3);
    ...
    // Add Light----------------------------------------------------------------
        ...
        //橙色 左侧光
           ...
           // lighting.rgb += Windows ? objmaskY2  * oLW : objmaskY3 * objmas2 * (1.0-cosTopMaskOL) * oL;
           //窗户
           ...
           // lighting.rgb = vec3(); 
             //橙色轮廓光
                ...
                vec3 OL6 = Windows?
                    vec3(0.0, 0.0, 0.0)
                    : 
                pow(skyboxColor.r,2.0) *(1.0-NyMask1)* frontMask6 * objmaskX3 * (1.0-objmas3_1)* Saturate(objectPos.z)  *  fresCol   ;
        ...
        //Fill_Light_01
            ...
            vec3 OL2 = objmaskX2 * NyMask * NxMask0 * frontMask3 * objmas2 * objmaskY3 * fresCol;
           ...
           //lighting.rgb = vec3(step(5.5,objectPos.y));
            ...
            vec4 FrontUpLighting =vec4( Saturate(mama2*0.7 + mamaHard*2.0  ) *( mama ) *0.55  * fresCol ,1.0);
           ...
           lighting = m_Add2Light (lighting,Spec,FrontUpLighting,matCol,0.9);
            ...
            // lighting = vec4(
            //     vec3(step(0.001,(ViewDirection.x-2.0)+(N.y)*2.0) + step(0.02,(1.0-ViewDirection.x-4.0)-(N.y)*2.0) )
        ...
        //橙色 底光2
            ...
            //右侧
                // vec3 OL3 = objmaskY * objmas * objmaskX * carTopMask * frontMask2 *fresCol ;
                ...
                //todo
                float yMask2 = (1.0-
                Saturate(smoothstep(0.5,2.0,objectPos.y+objectPos.z/25.0)));
                ...
                smoothstep(-3.1,2.1,
                    ...
                    objectPos.x +9.0  + 9.15*cos((objectPos.z-15.0)/10.2) + (objectPos.y+8.7)

                ) 
                )
                -( yMask * pow(objmas3,2.2)) + zMaskSideLight
                ...
                vec4 OL3 =vec4(
                    vec3(
                            ...
                            Saturate( CosMask-CosMask2*2.0 + smoothstep(0.0,1.0,(yMask2*(pow(objmas3,3.0)))*1.6))
                                 
                                 /  
                                 
                                 pow((Angle),1.9)*2.0* objmaskX*0.3    * objmaskX*5.5  * frontMask2 *Saturate( 1.2- SlashL)  * fresCol
                                 
                                 ),1.0); 

               
                lighting = m_Add2Light (lighting,Spec,OL3/6.0,matCol,0.72);
               // lighting.rgb = vec3( smoothstep(0.0,1.0,(yMask2*(pow(objmas3,3.0)))*1.6 ));
            ...
            //左侧
               ...
               float CosMaskL = 
                ...
                float fMask = (CosMaskL - CosMaskL2 +( yMaskL/2.0 * objmas3  ) ) * Saturate( smoothstep(3.0,9.1,objectPos.x)  );

                vec4 OL3L = vec4(fMask * frontMask2 * fresCol  ,1.0) ; 

                 if(Angle< 1.8)
                {
                    OL3L.rgb = SaturateV3(OL3L.rgb  * Saturate((Angle-1.2)*2.0))  ;
                   
                }
                  
                OL3L.rgb = SaturateV3((1.0 - SlashL) * OL3L.rgb);
                ...
                float SlashFL = Saturate( smoothstep(-3.8,3.0,-objectPos.y) - SlashL);
                vec4 OL4L = vec4( vec3(Saturate(SlashFL * pow(objmas,2.0) * frontMask2 ) * (1.0-objmaskX) * fresCol)  ,1.0) ; 
                
                lighting  = m_Add2Light (lighting,Spec,OL4L/3.2,matCol,0.72);


                
            // lighting.rgb = vec3(smoothstep(-6.0,3.0,-objectPos.y));   
        ...
        //后备箱光照
            ...
            lighting = Windows ? 
             ...
             //lighting.rgb + fBackCol 
                 m_Add2Light (lighting,Spec,vec4(fBackCol+addbright.rgb,1.0),matCol,1.2)
             : 
                m_Add2Light (lighting,Spec,vec4(fBackCol,1.0),matCol,0.5+log(Angle)*0.5);

            
            
          //  lighting.rgb = vec3( Saturate(smoothstep(-9.0,9.0,objectPos.z) ) );
  ...
  //  lighting.rgb = mix(vec3(0.0),lighting.rgb,m_texColor(AOTexture).r);

  
    if(Metal)
    {
        float metalMaskadd = smoothstep( 0.0,10.0,MakeCos(0.5,0.1,0.0,0.0,ViewDirection.z)*20.0);
        vec3 speedMetalViewDir =vec3(ViewDirection.x,ViewDirection.y-20.0,ViewDirection.z) ;
        vec3 I = normalize(speedMetalViewDir);
        vec3 viewR  = reflect(I, normalize(m_normal()));
        float metalMask =  step(0.001,(ViewDirection.x-pow((2.0*viewR.b),3.0))+(N.y)*4.2) + step(0.02,(1.0-ViewDirection.x-pow((4.0*viewR.b),3.0))-(N.y)*3.3) ;
        lighting.rgb += metalMask*0.5;
       // lighting.rgb -=  vec3(Saturate( smoothstep(0.92,0.99,N.z)));

        
    }
   // float MakeCos(float High,float cycle, float moveH , float moveV,float Target)
  
  //  lighting.rgb = vec3(viewR.b);

    if(AOTextureTirggle && CarHead)
    {
        lighting.rgb *=max( m_texColor(AOTexture).r,1.0-AOPower);
        float AoCarHeadMask = 1.0-step(5.0,objectPos.y);
        lighting.r += (1.0-max( m_texColor(AOTexture).r,1.0-AOPower))  * AoCarHeadMask;
        lighting.gb += (1.0-max( m_texColor(AOTexture).r,1.0-AOPower+0.6))  * AoCarHeadMask;
    }
    if(AOTextureTirggle && Back)
    {
        lighting.rgb += (1.0 - max( m_texColor(AOTexture).r,1.0-AOPower));
    }
    if(AOTextureTirggle && Metal)
    {
        lighting.rgb *=  max( m_texColor(AOTexture).r,1.0-AOPower);
    }
    if(AOTextureTirggle && Metal1)
    {
        lighting.rgb *=  max( m_texColor(AOTexture).r,1.0-AOPower);
        lighting.rgb = mix(lighting.rgb,vec3(AOPower),1.0-m_texColor(AOTexture).r);
    }
    if(AOTextureTirggle && Metal3)
    {
        lighting.rgb += (1.0 - max( m_texColor(AOTexture).r,1.0-AOPower));
        //lighting.rgb = vec3(0.0);
    }
    if( Plastic)
    {
        lighting.rgb = (Saturate (N.x*1.6) + Saturate(-N.x*1.6) - Saturate(smoothstep(6.1,7.0,objectPos.z))*0.4 ) *  SaturateV3( pow(lighting.rgb,vec3(0.89)) );

        if(AOTextureTirggle)
        {
            lighting.rgb = SaturateV3(lighting.rgb) - (1.0 - max( m_texColor(AOTexture).r,1.0-AOPower));
        }
    ...
    fCol = 
    Tire? 
    d_tire(lighting.rgb).rgb:lighting.rgb;
    //fCol =  vec3( );
...
#else
#endif
    gl_FragColor.rgb = Desaturation(fCol,DesaturationPower) * fA *1.3;
  //  gl_FragColor.rgb = vec3(pow(m_texColor(AOTexture).r,1.29));
    gl_FragColor.a = fA; 
}
