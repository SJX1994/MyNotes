...
float4 ApplyBlendMode(float3 diffuseLighting, float3 specularLighting, float opacity)
{
	...
	// « Add by HexaDrive.
	#if defined(_FUSE_BLENDMODE_MULTIPLY)
	diffuseLighting = lerp(float3(1.0, 1.0, 1.0), diffuseLighting, opacity);
	return float4(diffuseLighting, opacity);
	#endif
	// ª Add by HexaDrive.
...
float4 EvaluateAtmosphericScattering(PositionInputs posInput, float3 V, float4 inputColor)
{
	...
	// « Add by HexaDrive.
	#if defined(_FUSE_BLENDMODE_MULTIPLY)
	result.rgb = lerp(float3(1.0, 1.0, 1.0), result.rgb, (1.0 - volOpacity));
	#endif
	// ª Add by HexaDrive.
