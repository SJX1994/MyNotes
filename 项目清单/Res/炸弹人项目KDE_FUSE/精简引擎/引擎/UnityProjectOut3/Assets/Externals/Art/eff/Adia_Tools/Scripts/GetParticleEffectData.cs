...
public class GetParticleEffectData {
    ...
    public static string GetGetRuntimeMemorySizeStr(GameObject go)
    {
        ...
        return string.Format("贴图所占用的内存(Memory used by textures)：{0}   建议(suggest)：<{1}\n贴图数量(Number of textures)：{2}     建议(suggest)：<{3}", memorySizeStr, maxMemorySizeStr, FormatColorMax(textureCount, maxTextureCount), maxTextureCount);
    }
    ...
    public static string GetParticleSystemCount(GameObject go)
    {
        ...
        return string.Format("特效中所有粒子系统组件数量(Number of particle components)：{0}     建议(suggest)：<{1}", FormatColorMax(particleSystems.Length, max), max);
    }
    ...
    {
        int max = 10;
        return string.Format("DrawCall: {0}   最高：{1}   建议(suggest)：<{2}", FormatColorMax(GetOnlyParticleEffecDrawCall(), max), FormatColorMax(m_MaxDrawCall, max), max);
    }
    ...
    public static string GetPixRateStr(ParticleEffectScript particleEffectGo)
    {
        ...
        return string.Format("平均每像素overdraw率：{0}   建议(suggest)：<{1}", FormatColorMax(pixRate, max), max);
    }

    public static string GetParticleCountStr(ParticleEffectScript particleEffectGo)
    {
        int max = 50;
        return string.Format("粒子数量(particles)：{0}   最高：{1}   建议(suggest)：<{2}", FormatColorMax(particleEffectGo.GetParticleCount(), max), FormatColorMax(particleEffectGo.GetMaxParticleCount(), max), max);
    }
